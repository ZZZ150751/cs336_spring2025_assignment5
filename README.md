...
* **辅助方法 3：对数概率提取函数**
  ...
  通过模型前向传播获取 logits ，进而计算根据 `labels` 提取对应 token 的对数概率值。若布尔值为真，则同时调用辅助方法 2 返回每个 token 的熵。
...
回报 $R(\tau)$ 聚合了轨迹上的所有奖励，可表示为总奖励和或带折扣因子的累积奖励。
...
具体而言，GRPO 将传统方法中的 $R(\tau^{(i)}) - b(s_t^{(i)})$ 定义为优势 $A_t^{(i)}$。

在此设定下，基准可近似视为组内奖励均值。由于该基准以及分母的缩放因子与当前具体动作 $a_t$ 无关，这种变换在不改变梯度方向的前提下对梯度进行了有效缩放。
